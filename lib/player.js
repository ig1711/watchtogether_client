...
export default class Player extends EventEmitter {
    ...
    handleData = jsonData => {
        ...
        const idk = jsonData.toString('utf-8').trim().split('\n');
        for (let dk of idk) {
            const data = JSON.parse(dk);
    ...
    handleCommand(command) {
        const commands = ['Connect', 'Disconnect', 'Paused', 'Resumed', 'StatusRet'];
    ...
    log(text) {
        this.sendMessage({ username: 'Client Log', content: text, timestamp: Date.now() });
    }

    async getStatus() {
        const p = new Promise((resolve, reject) => {
            this.on('StatusRet', stat => {
                resolve(stat);
            });
        ...
        this.sendCommand({ 'Status': null });

        const status = await p.catch(null);

        this.removeAllListeners('StatusRet');
    ...
    connected() {
        this.sendCommand({ 'Connected': null });
    }

    disconnected() {
        this.sendCommand({ 'Disconnected': null });
    }
